...
function(_lm_import_lib pkg namespace lib_path)
    ...
    if("${lib_path}" IS_NEWER_THAN "${lib_cmake_file}")
        ...
        foreach(use IN LISTS lib__Uses)
            if(NOT use MATCHES "^(.+)/(.+)$")
        ...
        endforeach()
        foreach(link IN LISTS lib__Links)
            if(NOT use MATCHES "^(.+)/(.+)$")
                message(FATAL_ERROR "Cannot resolve invalid transitive usage on ${target_name}: ${use}")
                continue()
            endif()
            ...
            file(APPEND
                "${lib_cmake_file}.tmp"
                "set_property(TARGET [[${target_name}]] APPEND PROPERTY INTERFACE_LINK_LIBRARIES [[$<LINK_ONLY:${use_ns}::${use_lib}>]])\n"
                )
        endforeach()
        # Add verbatim links
...
function(export_library exp_target)
    ...
    foreach(lib IN LISTS libs)
        ...
        set(is_link_only FALSE)
        if(lib MATCHES "^\\$<LINK_ONLY:(.*)>$")
            set(lib "${CMAKE_MATCH_1}")
            set(is_link_only TRUE)
        endif()
        if(NOT TARGET "${lib}")
            message(WARNING
                "export_package() for '${ARG_NAME}' exports target '${exp_target}', "
        ...
        elseif(TARGET "${lib}" AND NOT is_imported)
            ...
            if(is_link_only)
                file(APPEND "${__lml_tmpl}" "Links: ${namespace}/${other_name}")
            else()
        ...
        elseif(lm_qual_name)
            if(is_link_only)
                file(APPEND "${__lml_tmpl}" "Links: ${lm_qual_name}\n")
            else()
